...
{
	if (line[x] == '1')
		mlx_put_image_to_window(game->mlx, game->ptr_win, game->wall_img, x
			* SIZE, y * SIZE);
	else if (line[x] == '0')
		mlx_put_image_to_window(game->mlx, game->ptr_win, game->ground_img, x
			* SIZE, y * SIZE);
	else if (line[x] == 'C')
	{
		mlx_put_image_to_window(game->mlx, game->ptr_win, game->collect_img, x
			* SIZE, y * SIZE);
	}
	else if (line[x] == 'E')
	{
		mlx_put_image_to_window(game->mlx, game->ptr_win, game->exit_img, x
			* SIZE, y * SIZE);
		game->exit_x = x;
		game->exit_y = y;
	}
	...
	{
		mlx_put_image_to_window(game->mlx, game->ptr_win, game->player_img, x
			* SIZE, y * SIZE);
		game->player_x = x;
		game->player_y = y;
	}
